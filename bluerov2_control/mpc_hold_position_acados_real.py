...
from bluerov2_control.models.fossen_bluerov2_model_real import build_bluerov2_fossen_model
...
class MPCHoldPositionAcadosReal(Node):
    def __init__(self):
        super().__init__("mpc_hold_position_acados_real")
        ...
        self.declare_parameter("codegen_dir", "/tmp/bluerov2_acados_codegen_real")
        ...
        self.get_logger().info(
            f"acados MPC robot_type = {str(self.get_parameter('robot_type').value)}"
        )
    ...
    def _build_mpc(self):
        ...
        if model_type == "fossen":
            x_sym, u_sym, xdot_fun, _ = build_bluerov2_fossen_model(
                Ts,
                current_speed_ned=(0.0, 0.0, 0.0),
        ...
        model = AcadosModel()
        model.name = f"bluerov2_{model_type}_{robot_type}_hold"
        model.x = x_sym
        model.u = u_sym
        model.p = p_sym
...
def main():
    rclpy.init()
    node = MPCHoldPositionAcadosReal()
    try:
        rclpy.spin(node)
