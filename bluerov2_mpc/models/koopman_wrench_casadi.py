...
class KoopmanEDMDcWrenchCasadi:
    """
    ...
    def create(self):
        ...
        u6 = cs.SX.sym('u', 6)            # column (6x1)

        z   = self._lift(x)               # (d x 1)
        zn  = self.A @ z + self.B @ u6    # (d x 1)
        x_n = zn[0:self.n]                # (12 x 1)

        self.f_disc = cs.Function('koopman_step', [x, u6], [x_n])
